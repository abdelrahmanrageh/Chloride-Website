export const versionsNames = ["windows10", "windows11", "tweaks"];

export const versions = {
  windows10Versions: [
    {
      version: "Beta 1.0.0 22h2 ",
      suitableFor: "Simple Daily Use",
      whatsNew:
        "0 bloatwares ,lower cpu processes ( 35 - 50 ) , lower ram usage ( 1.3 GB ), No Bluetooth , No Printing , 3.9GB iso",
      price: "Free",
      link: "https://www.mediafire.com/folder/de2p58pxbw1fr/Free+ChlorideO+10+1.0.0+22h2",
    },
    {
      version: "ChlorideOS 10 Enterprise LTSC Extreme ",
      suitableFor: "Best Option for you",
      whatsNew:
        "0 bloatwares ,lower cpu processes ( 50 - 60 ) , lower ram usage ( 0.7 - 0.9 GB ), 3.6GB iso",
      price: "Free",
      link: "https://www.youtube.com/@ChlorideOS/videos",
    },
    {
      version: "1.0.0 21H2 LTSC",
      suitableFor: "Best For Gaming",
      whatsNew:
        "0 bloatwares , lower cpu process ( 55 - 65 ) , lower ram usage ( 1.0 - 1.3 GB ), Only 7-8 GB of disk usage, Chloride Tweaks , Chloride Post Install Folder, 3.6GB iso",
      price: "With Tweaks Ultimate Plan",
      link: "",
    },
    {
      version: "1.0.0 1809 ",
      suitableFor: "Lightest Version, Best For Low End Devices",
      whatsNew:
        "75% Of bloatwares removed , lower cpu process ( 45 - 60 ) , lower ram usage ( 1.0 - 1.3 GB ), Only 7-8 GB of disk usage, Chloride Tweaks , Chloride Post Install Folder, 2.6GB iso With Tweaks Ultimate Plan",
      price: "With Tweaks Ultimate Plan",
      link: "https://chlorideos.netlify.app/",
    },
  ],

  windows11Versions: [
    {
      version: "Beta 1.0.0 22h2",
      suitableFor: "Simple Daily Use",
      whatsNew:
        "0 bloatwares ,lower cpu processes ( 35 - 50 ) , lower ram usage ( 1.3 GB ), No Bluetooth , No Printing , 3.9GB iso",
      price: "Free",
      link: "https://mega.nz/folder/opVQkLoB#I2Ths0zhZ9wdPBcihrvElA",
    },
    {
      version: "1.0.0 LTSC",
      suitableFor: "Best For Gaming",
      whatsNew:
        "0 bloatwares , lower cpu process ( 55 - 65 ) , lower ram usage ( 1.0 - 1.3 GB ), Only 7-8 GB of disk usage, Chloride Tweaks , Chloride Post Install Folder, 3.6GB iso",
      price: "With Tweaks Ultimate Plan",
      link: "https://chlorideos.netlify.app/",
    },
    {
      version: "1.0.0 22H2 Extreme ",
      suitableFor: "Lightest Version Ever, Best For Low End Devices",
      whatsNew:
        "80% OF bloatwares removed , lower cpu process ( 70 ) , lower ram usage ( 1.0 - 1.3 GB ), Only 13-16 GB of disk usage, Chloride Tweaks , Chloride Post Install Folder, 3.9GB iso",
      price: "With Tweaks Ultimate Plan",
      link: "https://chlorideos.netlify.app/",
    },
  ],

  tweaksVersions: [
    {
      version: "Standard Plan",
      suitableFor: "",
      whatsNew: `Gain up to 20% FPS boost with 25 essential tools and 100MB of scripts. Enjoy performance, network, power tweaks,
       GPU optimization, registry enhancements, deep PC debloating, and tailored Fortnite optimization for a competitive edge.`,
      price: "Free",
      link: "https://www.mediafire.com/folder/t32zpuvdwzrjd/Tweaks",
    },
    {
      version: "Gold Plan",
      suitableFor: "",
      whatsNew: `Unlock up to 35% FPS boost with 450 powerful tools and 615MB of scripts.
          Get custom Windows tweaks for performance, privacy, power, BIOS, and more.
          Remove bloatware, optimize the registry, and enhance efficiency with advanced power optimization.`,
      price: "3$",
      link: "https://chlorideos.netlify.app/",
    },
    {
      version: "Ultimate Plan",
      suitableFor: "",
      whatsNew: `
        Boost FPS by 35% with 450 premium tools and 615MB of advanced scripts. Get 100% free access to all ChlorideOS versions,
        offering optimizations for performance, privacy, network, power, BIOS, GPU, USB, and more. Remove unnecessary apps,
        optimize the registry, and enjoy deep PC debloating for a streamlined, efficient system. Elevate performance with
        advanced power optimization techniques.`,
      price: "5$",
      link: "https://chlorideos.netlify.app/",
    },
  ],

 

};
